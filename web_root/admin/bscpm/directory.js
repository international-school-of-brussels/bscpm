...
function openCloseDir(element, force) {
    if (force === undefined) {
        if (
            element.hasClass('directory-expanded') ||
            element.hasClass('directory-subCustom-expanded') ||
            element.hasClass('directory-custom-expanded')
        ) {
            force = 'collapse';
        } else {
            force = 'expand';
        ...
        items: {
            ...
            createFile: {
                ...
                callback: function (key, opt) {
                    ...
                    var message =
                        'Do not use any spaces and it must end with a file extension of txt, html , htm, js, javascript, json, css, rpt, xml, text, csv, tsv, or ftl.';
                    $('#new p.validateTips').html(message);
                    $('#new').dialog('open');
                    $('#newAssetType').val('file');
                    $('#newAssetPath').val(
                ...
                callback: function (key, opt) {
                    ...
                    if (containingItems.length > 0) {
                        ...
                        if (
                            confirm(
                                'There are ' +
                                    containingItems.length +
                                    ' custom files/directories contained in this folder. Are you sure you want to delete them all? There is no undo.'
                            )
                        ) {
...
function filterFiles() {
    ...
    if (showAll) {
        ...
        //expand directories
        $('#fileTree li[item-type=directory]')
            .has('li[display-status=true]')
            .each(function () {
                $(this).attr('display-status', true);
                if (openDirectories) {
                    openCloseDir($(this).children('div').children('div'), 'expand');
...
function loadFile(path, filename, custom, type, versionId) {
    ...
    var createTabLoadFile = function () {
        ...
        if (ext == 'png' || ext == 'gif' || ext == 'jpeg' || ext == 'jpg' || ext == 'ico') {
            ...
            editor.height(
                $(window).height() -
                    $('#editorButtonSet').height() -
                    $('#container').height() -
                    $('ul[role=tablist]').outerHeight(true) -
                    40
            );
...
function setupBasicEditor(editor, settings) {
    ...
    */
    editor.commands.addCommand({
        ...
        exec: function (editor) {
            $(
                '<div style="z-index:1010;"><iframe width="560" height="315" src="https://www.youtube.com/embed/6ykV3zaB0Uo?autoplay=1&start=356&end=365" frameborder="0" allowfullscreen></iframe></div>'
            )
                .dialog({
                    width: 589,
                    zIndex: 100,
