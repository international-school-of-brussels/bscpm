...
var urlPublishContent="/powerschool-sys-mgmt/custompages/publishCustomPageContent.action";
var urlRetrieveData="/powerschool-sys-mgmt/custompages/builtintext.action";
var urlCreate="/powerschool-sys-mgmt/custompages/createAsset.action";
var urlTree="/powerschool-sys-mgmt/custompages/tree.action";
var urlDelete="/powerschool-sys-mgmt/custompages/deleteCustomFile.action?";
var urlCustomizeStockAsset="/powerschool-sys-mgmt/custompages/customizeAsset.action";
...
function loggedIn(){
    var amIin=false;
    var url="/powerschool-sys-mgmt/custompages/tree.action";
    var parameters=[
...
/**
 * This checkt to make sure we are still logged in
 * 
...
function keyPath(path,file){
	...
	fileparts.pop();
	file=fileparts.join(".");
    return path.replace(/\//g,"_").substr(1)+"."+file;
}
...
function publishFile(editors,closeTab){
    ...
    for(var i=0;i<editors.length;i++){
        var selectTab=$(editors[i].container).attr('id').replace("-editor","");
        var selectedTabSelector="li[aria-controls="+selectTab+"]";
        var contents=editors[i].getValue();
        var customContentId=$(selectedTabSelector).attr("customcontentid");
        var a=$(selectedTabSelector).children("a")[0];
        var url="/powerschool-sys-mgmt/custompages/customizeAsset.action";
        var path=$(selectedTabSelector).attr("path")=="/"?"":$(selectedTabSelector).attr("path");
        var file=$(selectedTabSelector).attr("file");
        var fileType="file";
        var fullPath=path=="/"?"/"+file:path+"/"+file;
        var kPath=keyPath($(selectedTabSelector).attr("path"),$(selectedTabSelector).attr("file"));
            ...
            return $.ajax(url,{
                ...
                success:function(data){
                    if(data.returnMessage=="Custom file was created successfully"){
                        ...
                        $('#fileTree li[item-type="file"][item-path="'+fullPath.replace($(selectedTabSelector).attr("file"),"").substr(1)+'"][item-name="'+$(selectedTabSelector).attr("file")+'"]').attr("item-custom",true);
...
function publishRequest(a,contents,kPath,customContentId,fullPath,closeTab){
    var parameter=[
        ...
        {"name":"customContentId","value":customContentId},
        {"name":"customContentPath","value":fullPath}
    ];
...
function createItem(fileName,path,type,dialog,skip){
    ...
    url="/powerschool-sys-mgmt/custompages/createAsset.action";
    var parameter=[
...
function deleteFile(fullPath,selectedItem){
    var url="/powerschool-sys-mgmt/custompages/deleteCustomFile.action?";
    var parameter=[
            ...
            if(!(checkReturnData(data))){
                ...
                switch(data.returnMessage){
                    ...
                    case "The file was deleted sucessfully":
                        ...
                        closeFile( $("li[file-id='"+file+"']"),true);
                        //File deleted
                        selectedItem.remove();
        ...
        $.ajax(url,{
            ...
            success:function(data,status,jxhr){
                ...
                for(var i=0;i<pages.length;i++){
                    var a=$("<a />").attr("href","#");
                    var li=$("<li />").html(pages[i].name.substr(0,pages[i].name.lastIndexOf("."))).attr("item-fullpath",data.path+"/"+pages[i].name);
                    a.append(li);
                    ulElement.append(li);
                }
            },
...
function loggin(success,failure){
    ...
    $("#loginDia").dialog("option","buttons",[ 
        {
            ...
            click:function(){
                ...
                url="/powerschool-sys-mgmt/auth/j_acegi_security_check";
...
function selectedFiles(useHidden){
 ...
 function savePrefs(){
     ...
     var parameters={
         ...
         "ac":"setUserPref",
         "name":"bscpmPrefs",
         "value":JSON.stringify(prefJson)
     };
     var url="/admin/prefs/setuserpref.html";
     $.ajax(url,{
         dataType:"json",
         data:parameters,
         success:function(data){
